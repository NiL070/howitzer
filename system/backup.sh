...
cd /root
zip -r $IP-$date-$domain-howitzer.zip backup > /dev/null 2>&1
rclone copy /root/$IP-$date-$domain-howitzer.zip dr:backup/
url=$(rclone link dr:backup/$IP-$date-$domain-howitzer.zip)
...
rm -rf /root/backup
rm -r /root/$IP-$date-$domain-howitzer.zip
echo ""
